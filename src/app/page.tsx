...
export default function Home() {
  ...
  useEffect(() => {
    // API'lerimizden sitedeki kayıtlı kişi ve departman sayılarını çekiyoruz
    const fetchStats = async () => {
      try {
        // Promise.all kullanarak iki API'ye aynı anda (paralel) istek atıyoruz (hız için!)
        ...
        setStats({
          ...
          sectionsCount: (sectionsData.data || sectionsData).length || 0,
        });
      } catch (error) {
  ...
  return (
    <div className="max-w-6xl mx-auto mt-6 mb-20">
      ...
      {/* 1. DEVASA KARŞILAMA ALANI (HERO SECTION) */}
      ...
      {/* 2. CANLI İSTATİSTİK KARTLARI (STATS) */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 flex flex-col items-center justify-center text-center hover:-translate-y-1 transition-transform cursor-default">
          ...
          <div className="text-4xl font-black text-slate-800 flex items-center gap-3">
            Aktif 
            {/* Yanıp sönen yeşil nokta (Online efekti) */}
